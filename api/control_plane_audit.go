...
func (r CreateAuditExportSinkRequest) Validate() error {
	...
	case len(reference) < 1 || len(reference) > 1024:
		return validation.NewValidationFailedError(
			"endpointReference must contain between 1 and 1024 characters",
		)
	case strings.ContainsAny(reference, "\r\n"):
		return validation.NewValidationFailedError("endpointReference must not contain line breaks")
	case endpointReferenceContainsCredential(reference):
		return validation.NewValidationFailedError("endpointReference must not contain inline credentials")
...
func endpointReferenceContainsCredential(reference string) bool {
