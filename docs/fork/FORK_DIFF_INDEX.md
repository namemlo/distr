...
- Tests: Added parser/schema/checksum/secret-safety tests, duplicate JSON key tests, typed nested schema tests, repository-path tests, authority race tests, feature flag tests, handler tests, DB migration/authority tests, runbook publish guard tests, frontend service tests, and UI guard tests across supported resource pages. Live PostgreSQL authority repository tests require `DISTR_TEST_DATABASE_URL`.
