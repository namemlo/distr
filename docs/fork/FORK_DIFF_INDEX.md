...
170 and immutable, audit-bound protected-history artifact retention. The
Post-PR-100 scoped single-reviewer pilot exception owns migration 171 and
ADR-0087; it is generic, default-off, exact-scope, and evidence-labelled.
...
- Database changes: Migration 153 adds a mutable campaign draft root and immutable normalized revision, wave,
  member, and prerequisite evidence with guarded retention and downgrade.
...
- Status: Audit/export core implemented on an isolated synthetic branch; cross-domain instrumentation and the
  complete migration/live-database gate remain pending the ordered PR-063 through PR-077 integration.
- Upstream base: `ea6567b6` (synthetic PR-063 checkpoint).
