...
- Agent protocol changes: Docker agents advertise `distr.oci.job` version `1`, claim a task lease through the existing lease flow, heartbeat before and during job execution, enforce trusted `DISTR_OCI_JOB_ALLOWED_*` policy, run digest-only allowlisted OCI images with canonicalized read-only host mounts through Docker, stop containers on timeout/cancellation, and reuse deterministic containers only when trusted operation labels match for retry/reclaim/restart idempotency.
