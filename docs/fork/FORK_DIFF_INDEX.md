...
- Status: Implemented locally; strict validation package, validation API, authority persistence, mutation guards, frontend authority service/badge, examples, documentation, and ADR completed.
...
- Database changes: Added `ConfigAsCodeAuthority` and `ConfigAsCodeAuthorityAuditEvent` tables for org-scoped authority state and non-secret authority-change audit records.
...
- Tests: Added parser/schema/checksum/secret-safety tests, feature flag tests, handler tests, DB migration/authority tests, runbook publish guard tests, frontend service tests, and UI guard tests across supported resource pages. Live PostgreSQL authority repository tests require `DISTR_TEST_DATABASE_URL`.
