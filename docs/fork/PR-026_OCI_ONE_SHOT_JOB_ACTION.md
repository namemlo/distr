...
- removes stale secret staging directories, reuses exited deterministic containers, waits running deterministic containers, starts created deterministic containers, recreates unstarted created containers with missing secret mounts, and rejects unsupported existing-container states on retry, lease reclaim, or agent restart without replaying retained raw Docker logs
