...
- serializes inspect, secret staging, and container create per deterministic operation, with stale-owner reservation recovery after crashes, so concurrent same-operation agents adopt one execution instead of racing into false failures
