...
- The expand and contract changes never ship in the same release. This plan does not implement `prepare-contract`, a physical contract migration, canonical instant reads, column/index swaps, `ANALYZE`, raw/manual dirty-version `Force`, or post-contract rollback. The only permitted marker repair is the audited PR-054A recovery path, with `.Force(` confined to `internal/migrations/timestamp_dirty_recovery_runner.go`.
- No generic `--force`, vulnerability allowlist, vulnerability suppression, or scanner downgrade is permitted.
  Only the exact reviewed, expiring, fail-closed PR-050 vulnerability policy may accept findings.
...
- Modify: `docs/adr/0055-external-execution-timestamp-instants.md`
- Modify: `docs/superpowers/specs/2026-07-15-external-execution-timestamptz-hybrid-design.md`
- Modify: `docs/superpowers/plans/2026-07-15-external-execution-timestamp-expand.md`
...
- Vulnerability, license, and secret scanners remain mandatory. Generic suppression and allowlists remain
  prohibited; only the exact reviewed, expiring, fail-closed PR-050 policy may accept findings.
...
- Documentation: Added ADR-0055, the approved hybrid design, fenced Compose and dirty-recovery procedures,
  release/upgrade/smoke/security guidance, and PostgreSQL 16.14/18.4 gates.
...
- Compatibility notes: Expand continues reading legacy timestamps and writes paired legacy/instant values. Contract
  eligibility and canonical instant reads are separate later work. Dirty recovery is marker repair only and never
  starts Hub or clears the fence.
```
...
bash hack/test-server-compose-timestamp-expand.sh
DISTR_TIMESTAMP_TEST_GROUP=dirty-recovery bash hack/test-server-compose-timestamp-expand.sh
pnpm exec prettier --check .github/workflows/community-release-hardening.yaml docs/release/community-release-readiness.md docs/upgrade/community-release-upgrade-checklist.md docs/operations/server-docker-compose-deploy.md docs/operations/operator-smoke-test.md docs/security/release-hardening-checklist.md docs/fork/PR-054A_EXTERNAL_EXECUTION_TIMESTAMP_EXPAND.md docs/fork/FORK_DIFF_INDEX.md docs/fork/UPGRADE_GUIDE.md docs/adr/0055-external-execution-timestamp-instants.md docs/superpowers/specs/2026-07-15-external-execution-timestamptz-hybrid-design.md docs/superpowers/plans/2026-07-15-external-execution-timestamp-expand.md
git diff --check
```
...
timestamp dirty recovery compose orchestration tests passed
...
### Audited migration-138 dirty-marker recovery contract

The only supported dirty-marker entry point is:

```text
deploy.sh timestamp-expand-recover-dirty <approved-manifest-or-> <evidence-dir> <operator-identity> <reason>
```

The literal `-` is the no-manifest sentinel, not standard input. Operator identity is a stable non-secret
1-128-character safe identifier. Reason is one quoted, trimmed, printable, single-line, non-secret 1-256-character
argument without URL, credential, connection-string, token-assignment, or path material.
`DISTR_TIMESTAMP_DIRTY_RECOVERY_LOCK_TIMEOUT` defaults to `2m`, accepts a positive whole-number `s`, `m`, or `h`
duration, and must be exported in the host shell because the wrapper validates it before loading the Compose `.env`.

No-manifest recovery requires a complete captured evidence bundle and active `CAPTURED_WRITERS_STOPPED` fence that
predate migration. It cannot retrofit an interrupted ordinary zero-history `release`, because that path creates
neither record. Without them, do not manufacture evidence; keep writers stopped and restore the verified backup or
escalate.

Recovery chooses its marker from the exact catalog, never from the observed dirty version:

| Exact catalog/evidence state                                                     | Argument                                                                                     | Marker-repair result | Mandatory continuation                                                                                                                                                                                                                                                                                                                        |
| -------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------- | -------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `PREDECESSOR_137`, empty history, with the pre-existing staged fence/capture     | `-`                                                                                          | clean 137            | Retain the original immutable target handoff, then run normal cancel after its unchanged-schema checks pass. Stop there for rollback. To proceed forward, restore the exact target `DISTR_IMAGE_REF`, `DISTR_RELEASE_COMMIT`, and `DISTR_IMAGE_DIGEST` together from that handoff, run `check`, then restart ordinary zero-history `release`. |
| `PREDECESSOR_137`, non-empty history                                             | Exact approved root manifest                                                                 | clean 137            | Rerun normal apply with the same approved manifest and evidence.                                                                                                                                                                                                                                                                              |
| `EXPAND_138`, durable `ZERO_HISTORY`, with the pre-existing staged fence/capture | `-`                                                                                          | clean 138            | Remain stopped and fenced; escalate because no no-manifest finalizer exists.                                                                                                                                                                                                                                                                  |
| `EXPAND_138`, pre-apply `MANIFEST_REQUIRED` or stored `VERIFIED` tip             | Exact `APPROVED` root document or exact original `APPROVED` document matching the stored tip | clean 138            | Rerun normal apply/finalization with identical approved content and evidence.                                                                                                                                                                                                                                                                 |

Partial, mixed, unknown, and contract-gated catalogs remain refused. The wrapper reacquires the deployment lock;
revalidates the active fence, exact target image, captured evidence bundle, and manifest mode; proves all Hub writers
stopped; creates or validates the recovery plan; durably archives an exact interrupted result reservation; and
either reuses a valid retained result or invokes recovery Apply exactly once. It asserts writers remain stopped
before returning.
...
- every `timestamp-dirty-recovery-result.interrupted-NNN.partial` and `.sha256`.

The evidence directory remains deployment-user-owned mode `0700`; artifacts and sidecars remain deployment-user-owned
regular non-symlink files mode `0600`. Retry uses the same manifest mode and exact approved content/checksum,
evidence directory, identity, reason, and active fence. A different source pathname is acceptable only when it
supplies the same approved bytes and staged checksum. A valid retained plan may proceed after the marker is already
clean at its catalog-selected target; a valid retained result is reused without another operator call.

An Apply that observes the exact planned dirty status records `action: "FORCED"`. A retained-plan clean retry that
observes the catalog-selected marker already clean records `action: "OBSERVED_ALREADY_CLEAN"`. Both record
`result: "SUCCEEDED"` and a clean `postStatus` at `forceVersion`.

The confined recovery runner repairs only `schema_migrations`. It performs no DDL or external-execution data
mutation and never starts Hub, persists timestamp compatibility, or clears the writer fence. Direct marker edits,
raw/manual `migrate force`, and any `.Force(` outside
`internal/migrations/timestamp_dirty_recovery_runner.go` are prohibited. Marker repair never substitutes for normal
...
- [ ] Confirm the diff contains no contract migration, canonical instant reads, global pgx timestamp codec,
      unaudited/raw Force path, adopter name/host/credential, client DB access, or demo cleanup. The audited recovery
      path is permitted only with every `.Force(` call confined to
...
if ($implementationDiff | Select-String -Pattern 'prepare-contract|external-execution-timestamp-contract') { throw 'contract implementation leaked into expand release' }
$forceCalls = @(git grep -n '\.Force(' -- ':(glob)**/*.go')
$unexpectedForceCalls = @($forceCalls | Where-Object { $_ -notmatch '^internal/migrations/timestamp_dirty_recovery_runner\.go:' })
if ($unexpectedForceCalls.Count -ne 0) { throw 'Force call escaped the audited timestamp dirty-recovery runner' }
node hack/pr054a-validate-timestamp-expand.test.mjs
...
The required `release-gates` aggregator uses default fail-closed dependency semantics: it runs only after both
`fast-release-package` and `timestamp-expand-postgresql` succeed. Failed prerequisites remain workflow failures,
while the skipped dependent release gate cannot mask them.
...
unproved private configuration fails without echoing any token or the canary. Expected: no contract or
unaudited/raw Force implementation, every permitted Force call confined to the audited timestamp dirty-recovery
runner, and no newly added secret, local identity, private address, adopter-specific value, or unscannable binary
artifact.
...
- [ ] Audited dirty recovery accepts only exact `PREDECESSOR_137` or `EXPAND_138`, selects the force target from that
      catalog rather than the observed dirty marker, and retains the exact fence/image/evidence/manifest binding.
- [ ] Dirty-recovery plan/result sidecars and interrupted-result archives are durable and fail closed; an exact retry
      invokes Apply at most once or reuses a valid retained result without an operator call.
- [ ] Marker recovery performs no DDL or external-execution data mutation, starts no Hub, persists no compatibility,
      and clears no fence; `EXPAND_138`/`ZERO_HISTORY` remains stopped and fenced for escalation because no finalizer
      exists.
