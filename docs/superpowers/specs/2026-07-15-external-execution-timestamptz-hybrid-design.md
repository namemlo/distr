...
Unexpected migration failure has a narrow audited recovery command. It first checks whether the catalog is exactly the predecessor shape (transaction rolled back) or exactly the contracted shape (DDL committed but the clean-version write failed), revalidates the manifest checksum, then calls golang-migrate `Force` to the corresponding version and records the recovery identity, reason, prior dirty version, catalog checksum, and result. It refuses every mixed or unknown catalog state. This command is tested but is never used to bypass a failed preflight.
...
2. mandatory release workflow green without vulnerability suppression;
...
- full Go, vet, release lint, Angular, build, container, license, vulnerability, migration, and live-demo gates pass without exclusions.
