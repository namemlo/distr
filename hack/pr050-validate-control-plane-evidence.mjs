...
export function validateMigrationEvidence(report, expectedCommit, expectedPostgresVersion) {
  ...
  if (report.range?.from !== 138 || report.range?.to !== 162) {
    fail('migration report must cover the exact 138 through 162 range');
  }
  if (
  ...
  if (!Array.isArray(report.migrationFiles) || report.migrationFiles.length !== 25) {
    fail('migration report must contain exactly 25 migration file pairs');
  }
  ...
  if (
    ...
    report.coverage?.schemaUpgrade?.to !== 162 ||
    report.coverage?.schemaDown?.mode !== 'single-step' ||
    report.coverage?.schemaDown?.from !== 162 ||
    report.coverage?.schemaDown?.to !== 161 ||
