...
export function validateMigrationEvidence(report, expectedCommit, expectedPostgresVersion) {
  ...
  if (report.range?.from !== 138 || report.range?.to !== 169) {
    fail('migration report must cover the exact 138 through 169 range');
  }
  if (
  ...
  if (!Array.isArray(report.migrationFiles) || report.migrationFiles.length !== 32) {
    fail('migration report must contain exactly 32 migration file pairs');
  }
  ...
  if (
    ...
    report.coverage?.schemaUpgrade?.to !== 169 ||
    report.coverage?.schemaDown?.mode !== 'single-step' ||
    report.coverage?.schemaDown?.from !== 169 ||
    report.coverage?.schemaDown?.to !== 168 ||
