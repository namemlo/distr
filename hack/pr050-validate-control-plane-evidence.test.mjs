      ...
      return {
        ...
        checks:
          id === 'migration-file-integrity'
            ? [{description: 'exact migration pairs 138 through 162', count: 25, checksum: sha('inventory')}]
            : [
                {
...
test('migration evidence requires an intact 138-162 executable matrix from a clean source and disposable database', () => {
  assert.doesNotThrow(() => validateMigrationEvidence(migrationReport(), commit, '16.14'));

  for (const mutate of [
    (report) => (report.source.workingTreeDirty = true),
    (report) => (report.range.to = 161),
