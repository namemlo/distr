...
const privacyPackageVersionAtSyntaxIsSafe = (text, match, domain, localPart) => {
  if (!/^v?\d+\.\d+\.\d+(?:-[0-9a-z-]+(?:\.[0-9a-z-]+)*)?$/i.test(domain)) return false;
  const start = match.index;
  if (start === undefined) return false;
  const before = text.slice(0, start);
  const after = text.slice(start + match[0].length);
  const pathCoordinate = /^[A-Z0-9][A-Z0-9._-]*(?:\/[A-Z0-9][A-Z0-9._-]*)+$/i.test(localPart);
  const lockKey = /^[A-Z0-9][A-Z0-9._-]*$/i.test(localPart) && before.trim() === '' && /^:\s*(?:\{\})?\s*$/.test(after);
  return pathCoordinate || lockKey;
};
const privacyContainsNonReservedEmail = (text) => {
  ...
  for (const match of text.matchAll(candidate)) {
    ...
    if (privacyPackageVersionAtSyntaxIsSafe(text, match, domain, localPart)) continue;
...
const expectedMasterLedger = [
  ...
  '| PR-090                | 169                  | 0077              |',
  '| PR-091 through PR-092 | None                 | 0078 through 0079 |',
  '| PR-094                | None                 | 0081              |',
  '| PR-095                | 170                  | 0082              |',
...
const requireSnippetCount = (file, snippet, expected) => {
  const count = read(file).split(snippet).length - 1;
  if (count !== expected) {
    fail(`${file}: expected ${expected} occurrences of ${snippet}`);
  }
};

const requireContractSnippets = (text, snippets, message) => {
  if (snippets.some((snippet) => !text.includes(snippet))) fail(message);
};
const shellFunctionLines = (text, name) =>
  sectionLines(text, `${name}() {`, (line) => /^[A-Za-z_][A-Za-z0-9_]*\(\) [({]$/.test(line));

const goASTAuditSource = String.raw`package main
...
type forceSite struct {
	Path string
	Line int
}

type auditResult struct {
	ForceSites                []forceSite
	RecoverDirtyRegistrations int
}
...
func isRecoverDirtyRegistration(call *ast.CallExpr) bool {
	selector, ok := call.Fun.(*ast.SelectorExpr)
	if !ok || selector.Sel.Name != "AddCommand" || !isIdentifier(selector.X, "command") {
		return false
	}
	if len(call.Args) != 1 {
		return false
	...
	if !ok || !isIdentifier(constructor.Fun, "newMigrateRecoverDirtyCommand") || len(constructor.Args) != 1 {
		return false
	}
	runtime, ok := constructor.Args[0].(*ast.CompositeLit)
	return ok && isIdentifier(runtime.Type, "migrateRecoverDirtyRuntime") && len(runtime.Elts) == 0
}

func main() {
	if len(os.Args) != 2 {
		fmt.Fprintln(os.Stderr, "usage: go-ast-audit <repository-root>")
		os.Exit(2)
	}
	root, err := filepath.Abs(os.Args[1])
	if err != nil {
		fmt.Fprintln(os.Stderr, "invalid repository root")
		os.Exit(1)
	}
	result := auditResult{ForceSites: []forceSite{}}
	err = filepath.WalkDir(root, func(file string, entry fs.DirEntry, walkErr error) error {
		...
		if entry.Type()&os.ModeSymlink != 0 || entry.IsDir() || !strings.HasSuffix(entry.Name(), ".go") {
			return nil
		}
		...
		ast.Inspect(parsed, func(node ast.Node) bool {
			if selector, ok := node.(*ast.SelectorExpr); ok && selector.Sel.Name == "Force" {
				result.ForceSites = append(result.ForceSites, forceSite{
					Path: relative,
					Line: fileSet.Position(selector.Sel.Pos()).Line,
				})
			}
			if relative == "cmd/hub/cmd/migrate.go" {
				if call, ok := node.(*ast.CallExpr); ok && isRecoverDirtyRegistration(call) {
					result.RecoverDirtyRegistrations++
				}
			}
			return true
		})
	...
	if err != nil {
		fmt.Fprintln(os.Stderr, "repository AST walk failed")
		os.Exit(1)
	}
	if err := json.NewEncoder(os.Stdout).Encode(result); err != nil {
		fmt.Fprintln(os.Stderr, "AST result encoding failed")
		os.Exit(1)
	}
}
`;
const runGoASTAudit = () => {
  const auditDirectory = mkdtempSync(path.join(tmpdir(), 'pr054a-go-ast-'));
  const auditProgram = path.join(auditDirectory, 'main.go');
  try {
    writeFileSync(auditProgram, goASTAuditSource, {encoding: 'utf8', mode: 0o600});
    const execution = spawnSync('go', ['run', 'main.go', root], {
      cwd: auditDirectory,
      encoding: 'utf8',
    ...
    if (execution.error || execution.signal || execution.status !== 0) {
      fail('Go AST audit failed closed');
    }
    try {
      const result = JSON.parse(execution.stdout);
      if (!result || !Array.isArray(result.ForceSites) || !Number.isSafeInteger(result.RecoverDirtyRegistrations)) {
        fail('Go AST audit failed closed');
      }
      return result;
    } catch {
...
const goASTAudit = runGoASTAudit();

const dirtyRecoveryMigrateCommand = read('cmd/hub/cmd/migrate.go');
const dirtyRecoveryCLI = read('cmd/hub/cmd/migrate_recover_dirty.go');
if (goASTAudit.RecoverDirtyRegistrations !== 1) {
  fail('dirty recovery CLI contract mismatch');
}
requireContractSnippets(
  dirtyRecoveryMigrateCommand,
  ['command.AddCommand(newMigrateRecoverDirtyCommand(migrateRecoverDirtyRuntime{}))'],
  'dirty recovery CLI contract mismatch'
);
requireContractSnippets(
  dirtyRecoveryCLI,
  [
...
const dirtyRecoveryDeployFile = 'deploy/server-docker-compose/deploy.sh';
const dirtyRecoveryDeploy = read(dirtyRecoveryDeployFile);
const dirtyRecoveryFunctionLines = shellFunctionLines(dirtyRecoveryDeploy, 'timestamp_expand_recover_dirty');
if (!dirtyRecoveryFunctionLines) fail('dirty recovery operator command contract mismatch');
const dirtyRecoveryFunction = dirtyRecoveryFunctionLines.join('\n');
requireContractSnippets(
  dirtyRecoveryFunction,
  [
  ...
  'dirty recovery operator command contract mismatch'
);

const dirtyRecoveryDispatchLines = shellFunctionLines(dirtyRecoveryDeploy, 'dispatch_command');
if (!dirtyRecoveryDispatchLines) fail('dirty recovery dispatch contract mismatch');
const dirtyRecoveryDispatch = dirtyRecoveryDispatchLines.join('\n');
requireContractSnippets(
  dirtyRecoveryDispatch,
  [
  ...
  'dirty recovery dispatch contract mismatch'
);
requireContractSnippets(
  dirtyRecoveryDeploy,
  [
...
const dirtyRecoveryArtifactStateLines = shellFunctionLines(
  dirtyRecoveryDeploy,
  'timestamp_dirty_recovery_artifact_state'
);
if (!dirtyRecoveryArtifactStateLines) fail('dirty recovery retained artifact contract mismatch');
requireContractSnippets(
  dirtyRecoveryArtifactStateLines.join('\n'),
  ['0:0) printf ABSENT ;;', '1:0) printf FINAL_ONLY ;;', '1:1) printf COMPLETE ;;'],
  'dirty recovery retained artifact contract mismatch'
);
requireContractSnippets(
  dirtyRecoveryFunction,
  [
...
for (const forbidden of [
  ...
  if (new RegExp(`^\\s*${forbidden}(?:\\s|$)`, 'm').test(dirtyRecoveryFunction)) {
    fail('dirty recovery non-finalization contract mismatch');
  }
}
requireContractSnippets(
  dirtyRecoveryFunction,
  ['assert_hub_writers_stopped || return'],
  'dirty recovery non-finalization contract mismatch'
);

const dirtyRecoveryHarness = read('hack/test-server-compose-timestamp-expand.sh');
const dirtyRecoveryFocusedLines = sectionLines(
  dirtyRecoveryHarness,
  'if [[ "${DISTR_TIMESTAMP_TEST_GROUP:-}" == dirty-recovery ]]; then',
  (line) => line.startsWith('if [[ "${DISTR_TIMESTAMP_TEST_GROUP:-}" == ')
);
if (!dirtyRecoveryFocusedLines) fail('dirty recovery focused harness mismatch');
const expectedDirtyRecoveryFocusedLines = [
  'if [[ "${DISTR_TIMESTAMP_TEST_GROUP:-}" == dirty-recovery ]]; then',
...
const normalizedDirtyRecoveryFocusedLines = dirtyRecoveryFocusedLines
  .map((line) => line.trim())
  .filter((line) => line !== '');
if (!sameValues(normalizedDirtyRecoveryFocusedLines, expectedDirtyRecoveryFocusedLines)) {
  fail('dirty recovery focused harness mismatch');
}

const dirtyRecoveryForceRunner = 'internal/migrations/timestamp_dirty_recovery_runner.go';
if (
  goASTAudit.ForceSites.length !== 2 ||
  goASTAudit.ForceSites.some(
    (site) => !site || site.Path !== dirtyRecoveryForceRunner || !Number.isSafeInteger(site.Line) || site.Line < 1
  )
) {
  fail('migration Force confinement mismatch');
}
requireContractSnippets(
  read(dirtyRecoveryForceRunner),
  [
    'return engine.instance.Force(version)',
    'func forceTimestampDirtyRecoveryMarker(',
    'forceErr := engine.Force(int(version))',
  ],
  'migration Force confinement mismatch'
);
...
requireSnippets(forkDoc, [
  ...
  'application-integrity and accidental-delete guard',
  'deploymentplantarget_target_fk',
]);

const timestampExpandMigrationUp = 'internal/migrations/sql/138_external_execution_timestamp_expand.up.sql';
const timestampExpandMigrationDown = 'internal/migrations/sql/138_external_execution_timestamp_expand.down.sql';
requireSnippets(timestampExpandMigrationUp, [
  'CREATE TABLE ExternalExecutionTimestampDeletionTombstone',
  "CHECK (deletion_reason = 'ORGANIZATION_RETENTION')",
  'CREATE FUNCTION external_execution_timestamp_capture_deletion()',
  'CREATE TRIGGER ExternalExecution_timestamp_deletion_tombstone',
  'CREATE TRIGGER ExternalExecutionEvent_timestamp_deletion_tombstone',
  'CREATE TRIGGER ExternalExecutionTimestampDeletionTombstone_append_only',
  'CREATE TRIGGER ExternalExecutionTimestampDeletionTombstone_reject_truncate',
  'externalexecutiontimestampdeletiontombstone_operation_nonzero',
  'nonzero retention operation identity',
  'deletion_timestamp := clock_timestamp()',
]);
requireSnippets(timestampExpandMigrationDown, [
  'LOCK TABLE',
...
requireSnippets('internal/db/organization.go', [
...
requireSnippets('internal/migrations/preflight.go', [
  'downgrade crossing 138 is forbidden after timestamp retention',
  'downgrade crossing 138 is forbidden after ZERO_HISTORY timestamp rows',
]);
requireSnippets('internal/types/external_execution_timestamp.go', [
  'json:"resolvedShadowCount"',
  'json:"unresolvedShadowCount"',
  'json:"resolvedDeletedEvidenceCount"',
  'json:"unresolvedDeletedEvidenceCount"',
]);
for (const field of [
  'json:"resolvedShadowCount"',
  'json:"unresolvedShadowCount"',
  ...
  requireSnippetCount('internal/types/external_execution_timestamp.go', field, 2);
}
requireSnippets('internal/migrations/migrate.go', [
  'restoreTimestampDowngradeGuardStatus',
  'guarded timestamp schema is incomplete',
  'repairContext, repairCancel := context.WithTimeout',
]);
requireSnippets('internal/db/external_execution_timestamps.go', [
...
requireSnippets('docs/security/release-hardening-checklist.md', [
  'not a PostgreSQL privilege boundary',
  'Least-privilege runtime/migrator roles',
]);
...
requireExactNamedStep(
  'fast-release-package',
  fastReleaseLines,
  'Validate timestamp dirty recovery Compose orchestration',
  [
  ...
  'dirty recovery Compose orchestration gate mismatch'
);
requireJobSnippets(
  'fast-release-package',
  fastReleaseLines,
...
for (const required of [
  ...
  'required `release-gates` aggregator uses default fail-closed dependency semantics',
  'Failed prerequisites remain workflow failures',
