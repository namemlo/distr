    ...
    scenarios: scenarioIds.map((id) => {
      ...
      return {
        ...
        checks: id === 'migration-file-integrity'
          ? [{description: 'migration inventory', count: 30, checksum: sha('inventory')}]
          : [{
...
for release_contract in "$HELPER" "$RELEASE_WORKFLOW"; do
  grep -Fq 'range=138..167' "$release_contract" ||
    fail "release evidence contract is not aligned to migration 167: $release_contract"
