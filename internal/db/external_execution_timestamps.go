...
const externalExecutionTimestampDeletionTombstonesSQL = `
SELECT source_table, source_row_id, source_column, column_ordinal,
  CASE WHEN raw_is_null THEN NULL ELSE
    to_char(raw_value, 'YYYY-MM-DD"T"HH24:MI:SS.US') END AS raw_value,
  CASE WHEN instant_value IS NULL THEN NULL ELSE
  ...
  CASE WHEN row_created_instant IS NULL THEN NULL ELSE
    to_char(row_created_instant AT TIME ZONE 'UTC',
      'YYYY-MM-DD"T"HH24:MI:SS.US') || 'Z' END AS row_created_instant,
  deletion_operation_id, deletion_reason,
...
type externalExecutionTimestampDeletionTombstone struct {
	currentExternalExecutionTimestampCell
	DeletionOperationID uuid.UUID `db:"deletion_operation_id"`
...
const externalExecutionTimestampDeletionTombstoneTable = "externalexecutiontimestampdeletiontombstone"
...
var externalExecutionTimestampGuardSpecs = []externalExecutionTimestampGuardSpec{
	...
	{
		errorLabel:       "deletion tombstone guard",
		functionName:     "external_execution_timestamp_tombstone_append_only",
		functionBodyHash: "1edfa6735b7b4a19ee1ad8686fa986441e9f2d7d73dbce0781ae78a0fbb4039e",
		tableName:        externalExecutionTimestampDeletionTombstoneTable,
		triggerName:      "externalexecutiontimestampdeletiontombstone_append_only",
		triggerType:      27,
	},
	{
		errorLabel:       "execution deletion guard",
		functionName:     "external_execution_timestamp_capture_deletion",
		functionBodyHash: "bfffd0e15835e8da290f44cfc428e33d3718d8f684368ce40f5a1d8f42537410",
		tableName:        "externalexecution",
		triggerName:      "externalexecution_timestamp_deletion_tombstone",
		triggerType:      11,
	},
	{
		errorLabel:       "event deletion guard",
		functionName:     "external_execution_timestamp_capture_deletion",
		functionBodyHash: "bfffd0e15835e8da290f44cfc428e33d3718d8f684368ce40f5a1d8f42537410",
		tableName:        "externalexecutionevent",
		triggerName:      "externalexecutionevent_timestamp_deletion_tombstone",
		triggerType:      11,
	},
	{
	...
	{
		errorLabel:       "deletion tombstone truncate guard",
		functionName:     externalExecutionTimestampRejectTruncateFunction,
		functionBodyHash: externalExecutionTimestampRejectTruncateBodyHash,
		tableName:        externalExecutionTimestampDeletionTombstoneTable,
		triggerName:      "externalexecutiontimestampdeletiontombstone_reject_truncate",
		triggerType:      34,
	},
}

var externalExecutionTimestampBusinessTriggerSetSpecs = []externalExecutionTimestampTriggerSetSpec{
	{tableName: "externalexecution", triggerCount: 3},
	{tableName: "externalexecutionevent", triggerCount: 2},
...
WITH required_legacy(table_name, column_name, nullable) AS (
  ...
  VALUES
    ...
    ('externalexecutiontimestampdeletiontombstone', 'source_table', 'text', false),
    ('externalexecutiontimestampdeletiontombstone', 'source_row_id', 'uuid', false),
    ('externalexecutiontimestampdeletiontombstone', 'source_column', 'text', false),
    ('externalexecutiontimestampdeletiontombstone', 'column_ordinal', 'smallint', false),
    ('externalexecutiontimestampdeletiontombstone', 'raw_value', 'timestamp without time zone', true),
    ('externalexecutiontimestampdeletiontombstone', 'raw_is_null', 'boolean', false),
    ('externalexecutiontimestampdeletiontombstone', 'instant_value', 'timestamp with time zone', true),
    ('externalexecutiontimestampdeletiontombstone', 'row_created_instant', 'timestamp with time zone', true),
    ('externalexecutiontimestampdeletiontombstone', 'deletion_operation_id', 'uuid', false),
    ('externalexecutiontimestampdeletiontombstone', 'deletion_reason', 'text', false),
    ('externalexecutiontimestampdeletiontombstone', 'deleted_at', 'timestamp with time zone', false),
  ...
  VALUES
    ...
    ('externalexecutiontimestampdeletiontombstone',
      'externalexecutiontimestampdeletiontombstone_pkey', 'p',
      'PRIMARY KEY (source_table, source_row_id, source_column)'),
    ('externalexecutiontimestampdeletiontombstone',
      'externalexecutiontimestampdeletiontombstone_raw_null_match', 'c',
      'CHECK ((raw_is_null = (raw_value IS NULL)))'),
    ('externalexecutiontimestampdeletiontombstone',
      'externalexecutiontimestampdeletiontombstone_reason', 'c',
      'CHECK ((deletion_reason = ''ORGANIZATION_RETENTION''::text))'),
    ('externalexecutiontimestampdeletiontombstone',
      'externalexecutiontimestampdeletiontombstone_operation_nonzero', 'c',
      'CHECK ((deletion_operation_id <> ' ||
...
), required_expand_default(
  table_name, column_name, default_definition
) AS (
  VALUES
    ('externalexecutiontimestampdeletiontombstone', 'deleted_at',
      'CURRENT_TIMESTAMP')
)
SELECT
  ...
  (SELECT count(*) FROM required_expand_default required
   JOIN pg_class relation ON relation.relname = required.table_name
    AND relation.relkind = 'r'
   JOIN pg_namespace namespace_row ON namespace_row.oid = relation.relnamespace
    AND namespace_row.nspname = current_schema()
...
type externalExecutionTimestampSchemaStatusExpectation struct {
	MarkerVersion          uint
	CatalogVersion         uint
	Dirty                  bool
	MarkerAlreadyValidated bool
}
...
func readExternalExecutionTimestampSchemaContractInTx(
	...
	return readExternalExecutionTimestampSchemaContractWithStatusInTx(ctx, nil)
}

func readExternalExecutionTimestampSchemaContractWithStatusInTx(
	ctx context.Context,
	expected *externalExecutionTimestampSchemaStatusExpectation,
) (externalExecutionTimestampSchemaContract, error) {
	database := internalctx.GetDb(ctx)
	effectiveVersion, err := readExternalExecutionTimestampEffectiveSchemaVersion(
		ctx,
		database,
		expected,
	)
	...
	if shadowNameCount != 6 || shadowShapeCount != 6 || ordinaryTableCount != 5 ||
		expandColumnCount != 77 || expandConstraintCount != 19 ||
		expandDefaultCount != 1 {
		return externalExecutionTimestampSchemaContract{},
			fmt.Errorf("schema %d is not expand-compatible", effectiveVersion)
	}
...
FROM ExternalExecutionTimestampExpandState`).Scan(
	...
	return externalExecutionTimestampSchemaContract{
		CatalogVersion: uint(effectiveVersion), IdentitySourceVersion: sourceVersion,
	}, nil
}

func readExternalExecutionTimestampEffectiveSchemaVersion(
	ctx context.Context,
	database queryable.Queryable,
	expected *externalExecutionTimestampSchemaStatusExpectation,
) (int64, error) {
	if expected != nil && expected.MarkerAlreadyValidated {
		return int64(expected.CatalogVersion), nil
	}
...
SELECT to_regclass(format('%I.schema_migrations', current_schema()))
	...
	if !versionTableExists {
		return 0, errors.New("schema_migrations is absent")
	}
...
WHERE constraint_row.conrelid =
  ...
  AND pg_get_constraintdef(constraint_row.oid) = 'PRIMARY KEY (version)'`).Scan(
	...
	if expected == nil {
		...
		if version != int64(expected.MarkerVersion) || dirty != expected.Dirty {
			return 0, fmt.Errorf(
				"schema marker is version %d dirty=%t; expected version %d dirty=%t",
				version,
				dirty,
				expected.MarkerVersion,
				expected.Dirty,
			)
		}
		effectiveVersion = int64(expected.CatalogVersion)
	}
...
SELECT
  ...
  (SELECT count(*) FROM ExternalExecutionTimestampCellProvenance)`).Scan(
	...
	tombstones, err := readExternalExecutionTimestampDeletionTombstonesInTx(ctx)
	if err != nil {
		return nil, err
	}
	if err := validateExternalExecutionTimestampDeletionTombstones(
		tombstones,
	); err != nil {
		return nil, err
	}
	...
	deletedExecutionIDs := make(map[uuid.UUID]struct{})
	deletedEventIDs := make(map[uuid.UUID]struct{})
	currentKeys := make(map[string]struct{}, len(cells))
	for _, cell := range cells {
		...
		if err := verifyPostBaselineTimestampCell(
			cell,
	...
	for _, tombstone := range tombstones {
		key := timestampCellKey(
			tombstone.SourceTable,
			tombstone.SourceRowID,
			tombstone.SourceColumn,
			uint8(tombstone.ColumnOrdinal),
		)
		if _, exists := currentKeys[key]; exists {
			return nil, fmt.Errorf(
				"zero-history cell %s has both a source row and tombstone",
				key,
			)
		}
		if err := verifyPostBaselineTimestampCell(
			tombstone.currentExternalExecutionTimestampCell,
			state.TransitionedAt,
			"post-transition tombstone",
		); err != nil {
			return nil, err
		}
		switch tombstone.SourceTable {
		case "externalexecution":
			deletedExecutionIDs[tombstone.SourceRowID] = struct{}{}
		case "externalexecutionevent":
			deletedEventIDs[tombstone.SourceRowID] = struct{}{}
		}
	}
	return &types.ExternalExecutionTimestampReadiness{
		...
		PostTransitionPairCount: uint64(len(cells) + len(tombstones)),
		DeletionTombstoneRows:   uint64(len(tombstones)),
		DeletedExecutionCount:   uint64(len(deletedExecutionIDs)),
		DeletedEventCount:       uint64(len(deletedEventIDs)),
	}, nil
}
...
WHERE state='VERIFIED'`).Scan(&verifiedManifestCount); err != nil {
	...
	verified, err := verifyExternalExecutionTimestampManifestWithStatusInTx(
		ctx,
		*tip,
		expected,
	)
	...
	return &types.ExternalExecutionTimestampReadiness{
		...
		ResolvedShadowCount:            verified.ResolvedShadowCount,
		UnresolvedShadowCount:          verified.UnresolvedShadowCount,
		ResolvedDeletedEvidenceCount:   verified.ResolvedDeletedEvidenceCount,
		UnresolvedDeletedEvidenceCount: verified.UnresolvedDeletedEvidenceCount,
		DeletionTombstoneRows:          verified.DeletionTombstoneRows,
		DeletedExecutionCount:          verified.DeletedExecutionCount,
		DeletedEventCount:              verified.DeletedEventCount,
	}, nil
}
...
func checkExternalExecutionTimestampExpandReadinessInTx(
	...
	return checkExternalExecutionTimestampExpandReadinessWithStatusInTx(ctx, nil)
}

func checkExternalExecutionTimestampExpandReadinessWithStatusInTx(
	ctx context.Context,
	expected *externalExecutionTimestampSchemaStatusExpectation,
) (*types.ExternalExecutionTimestampReadiness, error) {
	contract, err := readExternalExecutionTimestampSchemaContractWithStatusInTx(
		ctx,
		expected,
	)
	if err != nil {
...
func CheckExternalExecutionTimestampExpandReadinessInTx(
	ctx context.Context,
	tx pgx.Tx,
	expectedMarkerVersion uint,
	expectedCatalogVersion uint,
	expectedDirty bool,
) (*types.ExternalExecutionTimestampReadiness, error) {
	if expectedMarkerVersion != 137 && expectedMarkerVersion != 138 {
		return nil, errors.New(
			"timestamp recovery marker version must be 137 or 138",
		)
	}
	if expectedCatalogVersion != 138 {
		return nil, errors.New(
			"timestamp recovery catalog version must be 138",
		)
	}
	return checkExternalExecutionTimestampExpandReadinessWithStatusInTx(
		internalctx.WithDb(ctx, tx),
		&externalExecutionTimestampSchemaStatusExpectation{
			MarkerVersion:  expectedMarkerVersion,
...
func CheckExternalExecutionTimestampExpandRecoveryReadinessInTx(
	ctx context.Context,
	tx pgx.Tx,
	expectedCatalogVersion uint,
) (*types.ExternalExecutionTimestampReadiness, error) {
...
func readExternalExecutionTimestampDeletionTombstonesInTx(
	ctx context.Context,
) ([]externalExecutionTimestampDeletionTombstone, error) {
	rows, err := internalctx.GetDb(ctx).Query(
		ctx,
		externalExecutionTimestampDeletionTombstonesSQL,
	)
	if err != nil {
		return nil, err
	}
	return pgx.CollectRows(
		rows,
		pgx.RowToStructByName[externalExecutionTimestampDeletionTombstone],
	)
}

func externalExecutionTimestampSourceRowKey(
	sourceTable string,
	sourceRowID uuid.UUID,
) string {
	return sourceTable + "/" + sourceRowID.String()
}

func validateExternalExecutionTimestampDeletionTombstones(
	tombstones []externalExecutionTimestampDeletionTombstone,
) error {
	type deletionGroup struct {
		operationID uuid.UUID
		deletedAt   string
		ordinals    map[int16]struct{}
	}
	groups := make(map[string]*deletionGroup)
	for _, tombstone := range tombstones {
		if tombstone.DeletionReason != "ORGANIZATION_RETENTION" ||
			tombstone.DeletionOperationID == uuid.Nil {
			return errors.New("deletion tombstone has invalid authorization")
		}
		if _, err := externalexecutiontimestamp.ParseInstant(
			tombstone.DeletedAt,
		); err != nil {
			return fmt.Errorf("deletion tombstone time is invalid: %w", err)
		}
		if tombstone.InstantValue != nil {
			if _, err := externalexecutiontimestamp.ParseInstant(
				*tombstone.InstantValue,
			); err != nil {
				return fmt.Errorf("deletion tombstone instant is invalid: %w", err)
			}
		}
		if tombstone.RowCreatedInstant != nil {
			if _, err := externalexecutiontimestamp.ParseInstant(
				*tombstone.RowCreatedInstant,
			); err != nil {
				return fmt.Errorf(
					"deletion tombstone creation instant is invalid: %w",
					err,
				)
			}
		}
		rawCell := types.ExternalExecutionTimestampRawCell{
			SourceTable:   tombstone.SourceTable,
			SourceRowID:   tombstone.SourceRowID,
			SourceColumn:  tombstone.SourceColumn,
			ColumnOrdinal: uint8(tombstone.ColumnOrdinal),
			RawValue:      tombstone.RawValue,
		}
		if _, err := externalexecutiontimestamp.ComputeRawCellChecksum(
			rawCell,
		); err != nil {
			return fmt.Errorf("deletion tombstone cell is invalid: %w", err)
		}
		rowKey := externalExecutionTimestampSourceRowKey(
			tombstone.SourceTable,
			tombstone.SourceRowID,
		)
		group := groups[rowKey]
		if group == nil {
			group = &deletionGroup{
				...
				ordinals:    make(map[int16]struct{}),
			}
			groups[rowKey] = group
		}
		if group.operationID != tombstone.DeletionOperationID ||
			group.deletedAt != tombstone.DeletedAt {
			return fmt.Errorf(
				"deletion tombstone row %s has inconsistent operation evidence",
				rowKey,
			)
		}
		group.ordinals[tombstone.ColumnOrdinal] = struct{}{}
	}
	for rowKey, group := range groups {
		sourceTable := strings.SplitN(rowKey, "/", 2)[0]
		expected := []int16{6}
		if sourceTable == "externalexecution" {
			expected = []int16{1, 2, 3, 4, 5}
		}
		if len(group.ordinals) != len(expected) {
			return fmt.Errorf(
				"deletion tombstone row %s is incomplete",
				rowKey,
			)
		}
		for _, ordinal := range expected {
			if _, exists := group.ordinals[ordinal]; !exists {
				return fmt.Errorf(
					"deletion tombstone row %s is incomplete",
					rowKey,
				)
...
func verifyPostBaselineTimestampCell(
	cell currentExternalExecutionTimestampCell,
	baseline time.Time,
	label string,
) error {
	if cell.RowCreatedInstant == nil {
		return fmt.Errorf("%s creation instant is absent", label)
	}
	created, err := externalexecutiontimestamp.ParseInstant(
		*cell.RowCreatedInstant,
	)
	if err != nil {
		return fmt.Errorf("%s creation instant is invalid: %w", label, err)
	}
	if created.Before(baseline) {
		return fmt.Errorf("%s creation predates verification", label)
	}
...
func readExternalExecutionTimestampDeletedPromotionAppliedAtInTx(
	ctx context.Context,
	latest types.ExternalExecutionTimestampManifest,
) (map[string]time.Time, error) {
	promoted := make(map[string]time.Time)
	current := latest
	for current.SupersedesManifestID != nil {
		var currentState types.ExternalExecutionTimestampManifestState
		var currentAppliedAt *time.Time
		if err := internalctx.GetDb(ctx).QueryRow(ctx, `
SELECT state, applied_at
FROM ExternalExecutionTimestampManifest
WHERE id=@id`,
			pgx.NamedArgs{"id": current.ID},
		).Scan(&currentState, &currentAppliedAt); err != nil {
			return nil, err
		}
		if currentState != types.ExternalExecutionTimestampManifestStateVerified ||
			currentAppliedAt == nil {
			return nil, errors.New(
				"manifest promotion child must be VERIFIED and applied",
			)
		}
		previous, state, err := readStoredExternalExecutionTimestampManifestInTx(
			ctx,
			*current.SupersedesManifestID,
		)
		if err != nil {
			return nil, err
		}
		if state != types.ExternalExecutionTimestampManifestStateVerified {
			return nil, errors.New(
				"manifest promotion ancestry must be VERIFIED",
			)
		}
		previousCells := make(
		...
		for _, cell := range current.Cells {
			...
			if !exists {
				return nil, fmt.Errorf(
					"manifest promotion cell %s has no ancestor",
					key,
				)
			}
			if previousCell.Decision ==
				...
				if _, duplicate := promoted[key]; duplicate {
					return nil, fmt.Errorf(
						"manifest promotion cell %s is promoted more than once",
						key,
					)
				}
				promoted[key] = currentAppliedAt.UTC()
			}
		}
		current = *previous
	}
...
func verifyDeletedHistoricalTimestampCell(
	provenance types.ExternalExecutionTimestampCellDecision,
	tombstone externalExecutionTimestampDeletionTombstone,
	lifecycleBaselineAt time.Time,
	provenanceOnlyPromotionAppliedAt time.Time,
) error {
	if !provenanceOnlyPromotionAppliedAt.IsZero() &&
		(provenance.Decision ==
			types.ExternalExecutionTimestampDecisionProven ||
			provenance.Decision ==
				types.ExternalExecutionTimestampDecisionAttested) &&
		tombstone.InstantValue == nil &&
		equalTimestampText(provenance.RawValue, tombstone.RawValue) {
		deletedAt, err := externalexecutiontimestamp.ParseInstant(
			tombstone.DeletedAt,
		)
		if err != nil {
			return fmt.Errorf("deletion tombstone time is invalid: %w", err)
		}
		if !deletedAt.Before(provenanceOnlyPromotionAppliedAt) {
			return errors.New(
	...
	return verifyHistoricalTimestampCell(
		provenance,
		tombstone.currentExternalExecutionTimestampCell,
		lifecycleBaselineAt,
	)
...
func verifyExternalExecutionTimestampManifestInTx(
	...
	return verifyExternalExecutionTimestampManifestWithStatusInTx(
		ctx,
		manifestID,
		nil,
	)
...
//nolint:gocyclo // Verification deliberately evaluates every independent manifest invariant in one report path.
func verifyExternalExecutionTimestampManifestWithStatusInTx(
	ctx context.Context,
	manifestID uuid.UUID,
	expected *externalExecutionTimestampSchemaStatusExpectation,
) (*types.ExternalExecutionTimestampVerificationReport, error) {
	...
	deletedPromotionAppliedAt, err := readExternalExecutionTimestampDeletedPromotionAppliedAtInTx(
		ctx,
		*manifest,
	)
	if err != nil {
	...
	tombstoneRows, err := readExternalExecutionTimestampDeletionTombstonesInTx(ctx)
	if err != nil {
		return nil, err
	}
	if err := validateExternalExecutionTimestampDeletionTombstones(
	...
	tombstones := make(
		map[string]externalExecutionTimestampDeletionTombstone,
		len(tombstoneRows),
	)
	executionIDs := make(map[uuid.UUID]struct{})
	eventIDs := make(map[uuid.UUID]struct{})
	...
	for _, tombstone := range tombstoneRows {
		key := timestampCellKey(
			tombstone.SourceTable,
			tombstone.SourceRowID,
			tombstone.SourceColumn,
		...
		if _, exists := current[key]; exists {
			return nil, fmt.Errorf(
				"cell %s has both a source row and authorized tombstone",
				key,
			)
		}
	...
	report := &types.ExternalExecutionTimestampVerificationReport{
		...
		DeletionTombstoneRows:   uint64(len(tombstoneRows)),
		DeletedExecutionCount:   uint64(len(deletedExecutionIDs)),
		DeletedEventCount:       uint64(len(deletedEventIDs)),
		RawSetChecksum:          manifest.RawCellChecksum,
		DecisionContentChecksum: manifest.DecisionContentChecksum,
	...
	for _, provenance := range manifest.Cells {
		...
		currentCell, sourceExists := current[key]
		tombstone, tombstoneExists := tombstones[key]
		if sourceExists && tombstoneExists {
			return nil, fmt.Errorf(
				"provenance cell %s has both a source row and authorized tombstone",
				key,
			)
		}
		if !sourceExists && !tombstoneExists {
			return nil, fmt.Errorf(
				"provenance cell %s has no source row or authorized tombstone",
				key,
			)
		}
		if tombstoneExists {
			currentCell = tombstone.currentExternalExecutionTimestampCell
		}
		var verifyErr error
		if tombstoneExists {
			verifyErr = verifyDeletedHistoricalTimestampCell(
				provenance,
				tombstone,
				lifecycleBaselineAt,
				deletedPromotionAppliedAt[key],
			)
		} else {
			verifyErr = verifyHistoricalTimestampCell(
				provenance,
				currentCell,
				lifecycleBaselineAt,
			)
...
func newlyPromotedTimestampCellsRequiringShadowFillInTx(
	...
	tombstoneRows, err := readExternalExecutionTimestampDeletionTombstonesInTx(
		ctx,
	)
	if err != nil {
		return nil, err
	...
	for _, nextCell := range next.Cells {
		...
		if sourceExists && tombstoneExists {
			return nil, fmt.Errorf(
				"promoted provenance cell %s has both source and tombstone",
				key,
			)
		}
		if !sourceExists && !tombstoneExists {
		...
		if tombstoneExists {
			if !equalTimestampText(previousCell.RawValue, tombstone.RawValue) {
				return nil, fmt.Errorf(
					"promoted deleted provenance cell %s raw timestamp changed",
					key,
				)
			}
			if tombstone.InstantValue != nil {
				return nil, fmt.Errorf(
					"promoted deleted provenance cell %s already has a shadow",
					key,
				)
			}
			continue
