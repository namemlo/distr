...
func TestExternalExecutionTimestampMigration138DeletionTombstonesAreAuthorizedAndAppendOnly(
	t *testing.T,
) {
	t.Run("delete without retention context is rejected", func(t *testing.T) {
		database := newMigrationTestDatabase(t)
		database.migrateTo(t, 137)
		_, eventID := insertHistoricalExecutionFixture(t, database)
		database.migrateTo(t, 138)
		...
		NewWithT(t).Expect(err).To(MatchError(ContainSubstring(
			"authorized retention context",
		)))
	})

	t.Run("authorized delete writes one complete immutable tombstone set", func(t *testing.T) {
		g := NewWithT(t)
		database := newMigrationTestDatabase(t)
		database.migrateTo(t, 137)
		executionID, eventID := insertHistoricalExecutionFixture(t, database)
...
SELECT set_config(
  ...
  true
		...
		err = database.pool.QueryRow(context.Background(), `
SELECT count(*),
       bool_and(deletion_operation_id = $1)
       AND bool_and(deletion_reason = 'ORGANIZATION_RETENTION')
       AND count(*) FILTER (
         WHERE source_table = 'externalexecution'
       ) = 5
       AND count(*) FILTER (
         WHERE source_table = 'externalexecutionevent'
       ) = 1
FROM ExternalExecutionTimestampDeletionTombstone`, operationID).Scan(
			&tombstoneCount,
			&complete,
		)
		g.Expect(err).NotTo(HaveOccurred())
		g.Expect(tombstoneCount).To(Equal(int64(6)))
		g.Expect(complete).To(BeTrue())

		_, err = database.pool.Exec(context.Background(), `
UPDATE ExternalExecutionTimestampDeletionTombstone
SET deletion_reason = 'CHANGED'`)
		g.Expect(err).To(MatchError(ContainSubstring("tombstone is append-only")))
		_, err = database.pool.Exec(
			context.Background(),
			`DELETE FROM ExternalExecutionTimestampDeletionTombstone`,
		)
		g.Expect(err).To(MatchError(ContainSubstring("tombstone is append-only")))
		_, err = database.pool.Exec(
			context.Background(),
			`TRUNCATE ExternalExecutionTimestampDeletionTombstone`,
		)
		g.Expect(err).To(MatchError(ContainSubstring(
			"timestamp evidence cannot be truncated",
		)))
	})

	t.Run("nil operation id rejects and rolls back source deletion", func(t *testing.T) {
		g := NewWithT(t)
		database := newMigrationTestDatabase(t)
		database.migrateTo(t, 137)
		_, eventID := insertHistoricalExecutionFixture(t, database)
...
SELECT set_config(
  ...
  true
		...
		g.Expect(tx.Rollback(context.Background())).To(Succeed())

		var eventCount, tombstoneCount int64
		g.Expect(database.pool.QueryRow(context.Background(), `
SELECT
  (SELECT count(*) FROM ExternalExecutionEvent WHERE id=$1),
  (SELECT count(*) FROM ExternalExecutionTimestampDeletionTombstone)`,
			eventID,
		).Scan(&eventCount, &tombstoneCount)).To(Succeed())
		g.Expect(eventCount).To(Equal(int64(1)))
		g.Expect(tombstoneCount).To(BeZero())
	})

	t.Run("transaction-local retention context resets after commit and rollback", func(t *testing.T) {
		g := NewWithT(t)
		database := newMigrationTestDatabase(t)
		database.migrateTo(t, 138)
		...
		assertContextUnset := func() {
			var unset bool
			g.Expect(connection.QueryRow(context.Background(), `
SELECT
  NULLIF(current_setting(
    'distr.external_execution_timestamp_deletion_reason', true
  ), '') IS NULL
  AND NULLIF(current_setting(
    'distr.external_execution_timestamp_deletion_operation_id', true
  ), '') IS NULL`).Scan(&unset)).To(Succeed())
			g.Expect(unset).To(BeTrue())
		}

		for _, finish := range []string{"commit", "rollback"} {
			tx, beginErr := connection.Begin(context.Background())
			g.Expect(beginErr).NotTo(HaveOccurred())
			_, err = tx.Exec(context.Background(), `
SELECT set_config(
  'distr.external_execution_timestamp_deletion_reason',
  'ORGANIZATION_RETENTION',
...
WHERE relation.relnamespace = to_regnamespace(current_schema())
  ...
  AND constraint_row.conname =
	...
	const (
		deletionReasonConstraint    = "CHECK ((deletion_reason = 'ORGANIZATION_RETENTION'::text))"
		deletionOperationConstraint = "CHECK ((deletion_operation_id <> " +
			"'00000000-0000-0000-0000-000000000000'::uuid))"
	)
	for constraintName, expected := range map[string]string{
		"externalexecutiontimestampdeletiontombstone_reason":            deletionReasonConstraint,
		"externalexecutiontimestampdeletiontombstone_operation_nonzero": deletionOperationConstraint,
	} {
		var definition string
		err = database.pool.QueryRow(context.Background(), `
SELECT pg_get_constraintdef(constraint_row.oid)
...
JOIN pg_class relation ON relation.oid = constraint_row.conrelid
WHERE relation.relnamespace = to_regnamespace(current_schema())
  AND relation.relname = 'externalexecutiontimestampdeletiontombstone'
  AND constraint_row.conname = $1`, constraintName).Scan(&definition)
		g.Expect(err).NotTo(HaveOccurred())
		g.Expect(definition).To(Equal(expected))
	}
	var deletedAtDefault string
	err = database.pool.QueryRow(context.Background(), `
SELECT pg_get_expr(attribute_default.adbin, attribute_default.adrelid)
FROM pg_attrdef attribute_default
JOIN pg_attribute attribute_row
  ON attribute_row.attrelid = attribute_default.adrelid
 AND attribute_row.attnum = attribute_default.adnum
WHERE attribute_default.adrelid =
      to_regclass(format(
        '%I.externalexecutiontimestampdeletiontombstone',
        current_schema()
      ))
  AND attribute_row.attname = 'deleted_at'`).Scan(&deletedAtDefault)
	g.Expect(err).NotTo(HaveOccurred())
	g.Expect(deletedAtDefault).To(Equal("CURRENT_TIMESTAMP"))
}
