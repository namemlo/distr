...
CREATE TABLE ExternalExecutionTimestampDeletionTombstone (
  ...
  row_created_instant TIMESTAMPTZ,
  deletion_operation_id UUID NOT NULL,
  deletion_reason TEXT NOT NULL,
  deleted_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (source_table, source_row_id, source_column),
  CONSTRAINT externalexecutiontimestampdeletiontombstone_reason
    CHECK (deletion_reason = 'ORGANIZATION_RETENTION'),
  CONSTRAINT externalexecutiontimestampdeletiontombstone_operation_nonzero
    CHECK (
      deletion_operation_id <>
        '00000000-0000-0000-0000-000000000000'::UUID
    ),
  CONSTRAINT externalexecutiontimestampdeletiontombstone_raw_null_match
    CHECK (raw_is_null = (raw_value IS NULL)),
  CONSTRAINT externalexecutiontimestampdeletiontombstone_allowlist CHECK (
    (
      source_table = 'externalexecution'
      AND (source_column, column_ordinal) IN (
        ('created_at', 1),
...
CREATE FUNCTION external_execution_timestamp_tombstone_append_only()
RETURNS TRIGGER LANGUAGE plpgsql AS $$
BEGIN
  RAISE EXCEPTION 'external execution timestamp deletion tombstone is append-only';
END;
$$;

CREATE TRIGGER ExternalExecutionTimestampDeletionTombstone_append_only
BEFORE UPDATE OR DELETE ON ExternalExecutionTimestampDeletionTombstone
FOR EACH ROW
EXECUTE FUNCTION external_execution_timestamp_tombstone_append_only();
...
BEGIN
  deletion_reason := current_setting(
    'distr.external_execution_timestamp_deletion_reason',
    true
  );
  BEGIN
    deletion_operation_id := NULLIF(current_setting(
  ...
  END;
  IF deletion_reason IS DISTINCT FROM 'ORGANIZATION_RETENTION' THEN
    RAISE EXCEPTION 'external execution timestamp deletion requires authorized retention context';
  END IF;
  IF deletion_operation_id IS NULL
     OR deletion_operation_id =
       '00000000-0000-0000-0000-000000000000'::UUID THEN
    RAISE EXCEPTION 'external execution timestamp deletion requires nonzero retention operation identity';
  END IF;
  deletion_timestamp := clock_timestamp();

  CASE TG_TABLE_NAME
    WHEN 'externalexecution' THEN
      INSERT INTO ExternalExecutionTimestampDeletionTombstone (
        source_table, source_row_id, source_column, column_ordinal,
        raw_value, raw_is_null, instant_value, row_created_instant,
        deletion_operation_id, deletion_reason, deleted_at
      )
      SELECT
        'externalexecution', OLD.id, cell.source_column,
        cell.column_ordinal, cell.raw_value, cell.raw_value IS NULL,
        cell.instant_value, OLD.created_at_instant,
        deletion_operation_id, deletion_reason, deletion_timestamp
      FROM (VALUES
        ('created_at'::TEXT, 1::SMALLINT,
          OLD.created_at, OLD.created_at_instant),
        ('updated_at'::TEXT, 2::SMALLINT,
          OLD.updated_at, OLD.updated_at_instant),
        ('started_at'::TEXT, 3::SMALLINT,
          OLD.started_at, OLD.started_at_instant),
        ('completed_at'::TEXT, 4::SMALLINT,
          OLD.completed_at, OLD.completed_at_instant),
        ('callback_deadline_at'::TEXT, 5::SMALLINT,
          OLD.callback_deadline_at, OLD.callback_deadline_at_instant)
    ...
    WHEN 'externalexecutionevent' THEN
      INSERT INTO ExternalExecutionTimestampDeletionTombstone (
        ...
        'externalexecutionevent', OLD.id, 'created_at', 6,
        OLD.created_at, FALSE, OLD.created_at_instant,
        OLD.created_at_instant, deletion_operation_id, deletion_reason,
        deletion_timestamp
      );
    ELSE
      RAISE EXCEPTION 'external execution timestamp deletion guard is attached to unsupported table %',
        TG_TABLE_NAME;
  END CASE;
  RETURN OLD;
END;
...
BEFORE DELETE ON ExternalExecution
FOR EACH ROW
EXECUTE FUNCTION external_execution_timestamp_capture_deletion();

CREATE TRIGGER ExternalExecutionEvent_timestamp_deletion_tombstone
BEFORE DELETE ON ExternalExecutionEvent
FOR EACH ROW
EXECUTE FUNCTION external_execution_timestamp_capture_deletion();
...
BEFORE TRUNCATE ON ExternalExecutionTimestampDeletionTombstone
FOR EACH STATEMENT
EXECUTE FUNCTION external_execution_timestamp_reject_truncate();
